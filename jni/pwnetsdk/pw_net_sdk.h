...
typedef struct tcaudioe_blk_array_tags
{
	...
	bool            blk_used;
	unsigned int    frm_blk_seq; 
	bool            blkstate[AUDIO_STATE];  
	audio_inner_blk blkdata[AUDIO_STATE];
}audio_blk;
...
// get alarm pic
long	PW_NET_GetAlarmPic(int nCamera_id,int nalarmtype,int nalarmcode,int nccid,int nstarttime,int nendtime); // -1 failure  0  no pic   1 succcess  2 no tf card
