...
static int avi_add_chunk( avi_t *AVI, unsigned char *tag, unsigned char *data, int length, long &AVI_Error)
{
	unsigned char c[8];
...
static int avi_add_index_entry(avi_t *AVI, unsigned char *tag, long flags, unsigned long pos, unsigned long len, long &AVI_errno)
{
	void *ptr;
...
int CAvi::AVI_open_output_file(char * filename)
{
	int i;
	...
	m_pstAviPara = (avi_t *) malloc(sizeof(avi_t));
	if(m_pstAviPara==0)
	{
		m_lAVI_errno = AVI_ERR_NO_MEM;
		return 0;
	}
	memset((void *)m_pstAviPara,0,sizeof(avi_t));
...
#ifdef WIN32
	m_pstAviPara->fdes = open(filename, O_RDWR|O_CREAT|O_BINARY, (S_IRUSR | S_IWUSR) &~ mask);
#else
	m_pstAviPara->fdes = open(filename, O_RDWR|O_CREAT, (S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH) &~ mask);
#endif
	if (m_pstAviPara->fdes < 0)
	{
		m_lAVI_errno = AVI_ERR_OPEN;
		free(m_pstAviPara);
		return 0;
	}
	...
	i = avi_write(m_pstAviPara->fdes,(char *)AVI_header,HEADERBYTES);
	if (i != HEADERBYTES)
	{
		close(m_pstAviPara->fdes);
		m_lAVI_errno = AVI_ERR_WRITE;
		free(m_pstAviPara);
		return 0;
	}

	m_pstAviPara->pos  = HEADERBYTES;
	m_pstAviPara->mode = AVI_MODE_WRITE; /* open for writing */
...
void CAvi::AVI_set_video(int width, int height, double fps, char *compressor)
{
	/* may only be called if file is open for writing */
	if(!m_pstAviPara)	return;
	avi_t *AVI = m_pstAviPara;
...
void CAvi::AVI_set_audio( int channels, long rate, int bits, int format, long mp3rate)
{
	/* may only be called if file is open for writing */
	if(!m_pstAviPara)	return;
	avi_t *AVI = m_pstAviPara;
...
int CAvi::avi_update_header()
{
	if(!m_pstAviPara)	return -1;
	avi_t *AVI = m_pstAviPara;
...
static int avi_close_output_file(avi_t *AVI, long &AVI_errno)
{
	...
	ret = avi_add_chunk(AVI, (unsigned char *)"idx1", (unsigned char *)AVI->idx, AVI->n_idx*16, AVI_errno);
	hasIndex = (ret==0);
...
static int avi_write_data(avi_t *AVI, char *data, unsigned long length, int audio, int keyframe, long &AVI_errno)
{
	int n;
	...
	if(audio)
		n = avi_add_index_entry(AVI,astr,0x00,AVI->pos,length, AVI_errno);
	else
		n = avi_add_index_entry(AVI,(unsigned char *)"00db",((keyframe)?0x10:0x0),AVI->pos,length, AVI_errno);
	...
	if(audio)
		n = avi_add_chunk(AVI,astr,(unsigned char *)data,length, AVI_errno);
	else
		n = avi_add_chunk(AVI,(unsigned char *)"00db",(unsigned char *)data,length, AVI_errno);
...
int CAvi::AVI_write_frame( char *data, long bytes, int keyframe)
{
	if(!m_pstAviPara)	return -1;
	avi_t *AVI = m_pstAviPara;	unsigned long pos;

	if(AVI->mode==AVI_MODE_READ) { m_lAVI_errno = AVI_ERR_NOT_PERM; return -1; }

	pos = AVI->pos;

	if(avi_write_data(AVI,data,bytes,0,keyframe, m_lAVI_errno)) return -1;
...
int CAvi::AVI_dup_frame()
{
	...
	if(avi_add_index_entry(AVI,(unsigned char *)"00db",0x10,AVI->last_pos,AVI->last_len, m_lAVI_errno)) return -1;
	AVI->video_frames++;
	AVI->must_use_index = 1;
	return 0;
}

int CAvi::AVI_write_audio( char *data, long bytes)
{
	if(!m_pstAviPara)	return -1;
	avi_t *AVI = m_pstAviPara;
	if(AVI->mode==AVI_MODE_READ) { m_lAVI_errno = AVI_ERR_NOT_PERM; return -1; }

	if( avi_write_data(AVI,data,bytes,1,0, m_lAVI_errno) ) return -1;
	AVI->track[AVI->aptr].audio_bytes += bytes;
	return 0;
}


int CAvi::AVI_append_audio(char *data, long bytes)
{
...
long CAvi::AVI_bytes_remain()
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
...
int CAvi::AVI_set_audio_track(int track)
{
...
int CAvi::AVI_close()
{
	...
	if(AVI->mode == AVI_MODE_WRITE)
		ret = avi_close_output_file(AVI, m_lAVI_errno);
	else
		ret = 0;
...
// }
 ...
 int CAvi::AVI_open_input_file(const char *filename, int getIndex)
{
	m_lAVI_errno = 0;
	/* Create avi_t structure */
	m_pstAviPara = (avi_t *) malloc(sizeof(avi_t));
	if(m_pstAviPara==NULL)
	{
		m_lAVI_errno = AVI_ERR_NO_MEM;
		return 0;
	}
	...
	m_pstAviPara->mode = AVI_MODE_READ; /* open for reading */

	/* Open the file */

#ifdef WIN32  
	m_pstAviPara->fdes = open(filename,O_RDONLY|O_BINARY);
#else
	m_pstAviPara->fdes = open(filename,O_RDONLY);
#endif
	if(m_pstAviPara->fdes < 0)
	{
		m_lAVI_errno = AVI_ERR_OPEN;
	...
	avi_parse_input_file(getIndex);

	m_pstAviPara->aptr=0; //reset  
	if(m_lAVI_errno)
	{
		AVI_close();
		m_pstAviPara = NULL;
...
int CAvi::AVI_open_fd(int fd, int getIndex)
{
	/* Create avi_t structure */
	m_lAVI_errno = 0;
...
int CAvi::avi_parse_input_file(int getIndex)
{
			...
			if(idx_type == 0)
			{
				...
				while(1)
				{
					...
					if( ( (data[2]=='d' || data[2]=='D') &&
						...
						(data[3]=='b' || data[3]=='B') ) )
					{
						avi_add_index_entry(AVI,(unsigned char *)data,0,lseek(AVI->fdes,0,SEEK_CUR)-8,n, m_lAVI_errno);
					}
...
long CAvi::AVI_video_frames()
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
	return AVI->video_frames;
}
int  CAvi::AVI_video_width()
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
	return AVI->width;
}
int  CAvi::AVI_video_height()
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
	return AVI->height;
}
double CAvi::AVI_frame_rate()
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
	return AVI->fps;
}
char* CAvi::AVI_video_compressor()
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
...
long CAvi::AVI_audio_codech_offset()
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
...
long CAvi::AVI_audio_codecf_offset()
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
...
long  CAvi::AVI_video_codech_offset()
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
...
long  CAvi::AVI_video_codecf_offset()
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
...
long CAvi::AVI_frame_size(long frame)
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
	if(AVI->mode==AVI_MODE_WRITE) { m_lAVI_errno = AVI_ERR_NOT_PERM; return -1; }
	if(!AVI->video_index)         { m_lAVI_errno = AVI_ERR_NO_IDX;   return -1; }

	if(frame < 0 || frame >= AVI->video_frames) return 0;
	return(AVI->video_index[frame].len);
}

long CAvi::AVI_audio_size(long frame)
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
	if(AVI->mode==AVI_MODE_WRITE) { m_lAVI_errno = AVI_ERR_NOT_PERM; return -1; }
	if(!AVI->track[AVI->aptr].audio_index)         { m_lAVI_errno = AVI_ERR_NO_IDX;   return -1; }

	if(frame < 0 || frame >= AVI->track[AVI->aptr].audio_chunks) return 0;
	return(AVI->track[AVI->aptr].audio_index[frame].len);
}

long CAvi::AVI_get_video_position(long frame)
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
	if(AVI->mode==AVI_MODE_WRITE) { m_lAVI_errno = AVI_ERR_NOT_PERM; return -1; }
...
int CAvi::AVI_set_video_position(long frame)
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;	if(AVI->mode==AVI_MODE_WRITE) { m_lAVI_errno = AVI_ERR_NOT_PERM; return -1; }
	if(!AVI->video_index)         { m_lAVI_errno = AVI_ERR_NO_IDX;   return -1; }
...
int CAvi::AVI_set_audio_bitrate(long bitrate)
{
	if(!m_pstAviPara)	return 0;
	avi_t *AVI = m_pstAviPara;
	if(AVI->mode==AVI_MODE_READ) { m_lAVI_errno = AVI_ERR_NOT_PERM; return -1; }
...
BOOL CAvi::AVI_check_key_frame(int *keyframe)
{

	if(!m_pstAviPara)	return FALSE;
	avi_t *AVI = m_pstAviPara;
	if(AVI->mode==AVI_MODE_WRITE) { m_lAVI_errno = AVI_ERR_NOT_PERM; return FALSE; }
	if(!AVI->video_index)         { m_lAVI_errno = AVI_ERR_NO_IDX;   return FALSE; }

	if(AVI->video_pos < 0 || AVI->video_pos >= AVI->video_frames) return FALSE;
...
long CAvi::AVI_read_frame(char *vidbuf, int *keyframe)
{
	long n;
...
int CAvi::AVI_set_audio_position(long byte)
{
	long n0, n1, n;
...
long CAvi::AVI_read_audio(char *audbuf, long bytes)
{
...
long CAvi::AVI_read_audio_chunk(char *audbuf)
{
	long pos, left;
	if(!m_pstAviPara)	return -1;
	avi_t *AVI = m_pstAviPara;
...
int CAvi::AVI_read_data(char *vidbuf, long max_vidbuf,
...
static int num_avi_errors = sizeof(avi_errors_c)/sizeof(char*);

static char error_string[4096];

void CAvi::AVI_print_error(char *str)
{
	int aerrno;

	aerrno = (m_lAVI_errno>=0 && m_lAVI_errno<num_avi_errors) ? m_lAVI_errno : num_avi_errors-1;

	fprintf(stderr,"%s: %s\n",str,avi_errors_c[aerrno]);

	/* for the following errors, perror should report a more detailed reason: */

	if(m_lAVI_errno == AVI_ERR_OPEN ||
		m_lAVI_errno == AVI_ERR_READ ||
		m_lAVI_errno == AVI_ERR_WRITE ||
		m_lAVI_errno == AVI_ERR_WRITE_INDEX ||
		m_lAVI_errno == AVI_ERR_CLOSE )
	{
		perror("REASON");
	}
...
char *CAvi::AVI_strerror()
{
	...
	if(m_lAVI_errno == AVI_ERR_OPEN ||
		...
		m_lAVI_errno == AVI_ERR_CLOSE )
	{
		sprintf(error_string,"%s - %s",avi_errors_c[aerrno],strerror(errno));
		return error_string;
	}
	else
	{
		return avi_errors_c[aerrno];
	}
}

uint64_t CAvi::AVI_max_size()
{
