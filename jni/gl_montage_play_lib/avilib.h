...
int AVI_Be_End(avi_t *AVI);
long AVI_get_frame_len(avi_t *AVI);
...
public:
	...
	int AVI_open_output_file(char * filename);
	...
	int AVI_append_audio(char *data, long bytes);
	long AVI_bytes_remain();
	int AVI_close();
	long AVI_bytes_written();

	int AVI_open_input_file(const char *filename, int getIndex);
	int AVI_open_fd(int fd, int getIndex);
	int avi_parse_input_file(int getIndex);
	long AVI_audio_mp3rate();
	...
	long AVI_audio_chunks();

	long AVI_max_video_chunk();

	long AVI_frame_size(long frame);
	long AVI_audio_size(long frame);
	int AVI_seek_start();

	int AVI_set_video_position(long frame);
	long AVI_get_video_position(long frame);

	int AVI_Be_End();
	long AVI_get_frame_len();
	bool AVI_check_key_frame(int *keyframe);
	long AVI_read_frame(char *vidbuf, int *keyframe);

	int AVI_set_audio_position(long byte);
	int AVI_set_audio_bitrate(long bitrate);

	long AVI_read_audio(char *audbuf, long bytes);
	long AVI_read_audio_chunk(char *audbuf);

	long AVI_audio_codech_offset();
	long AVI_audio_codecf_offset();
	long AVI_video_codech_offset();
	long AVI_video_codecf_offset();

	int AVI_read_data(char *vidbuf, long max_vidbuf, char *audbuf, long max_audbuf, long *len);
	...
	int AVI_set_audio_track(int track);
	int AVI_get_audio_track();
	int AVI_audio_tracks();
};

// struct GVM_AVIStreamHeader {
//   long  fccType;
//   long  fccHandler;
//   long  dwFlags;
//   long  dwPriority;
