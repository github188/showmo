...
typedef struct PMP_MONTAGE_PLAY
{
	...
	PMP_API GBool	pw_mp_ReleaseDataSourceID(
		PMP_IN	GInt32				lSourceID
		);
/*---------------------------------------------------
*	Decode API 
	...
	*Set stream info
	pstrInfoName ==
		...
		"audio_samplerate"
			pstrValue == "n"---- > 0 
		"video_fps_rate"
			pstrValue == "f"---- > 1.0f: play fast; < 1.0f: play slow
...
// 		);
	...
	PMP_API GBool	pw_mp_StoreSth(
		PMP_IN	GInt32			lSourceID,
		PMP_IN	const GInt8*	pcBuff,
		PMP_IN	const GUInt32	dwBuffLen,
		PMP_IN	const GChar*	pstrSthName
		);
	PMP_API GBool	pw_mp_WatchSth(
		PMP_IN	GInt32			lSourceID,
		PMP_OUT	GInt8**			ppcBuff,
		PMP_OUT	GUInt32*		pdwBuffLen,
		PMP_IN	const GChar*	pstrSthName
		);
	PMP_API GBool	pw_mp_TakeSth(
		PMP_IN	GInt32			lSourceID,
		PMP_OUT	GInt8**			ppcBuff,
		PMP_OUT	GUInt32*			pdwBuffLen,
		PMP_IN	const GChar*	pstrSthName
...
*--------------------------------------------------*/
	...
	/*-- Set the parameters. 
	*Set whether get pano_info again.
		pstrInfoName ==
			"pano_info_ready"
			pstrValue ==
				"y"----ready
	...
	*Run gl_pattern com.
		pstrInfoName ==
			"gl_pattern_play"
	*Set gl_pattern get pano_info from which data_source, "single" is default.
		pstrInfoName ==
			"pano_source_name"
			pstrValue == data_source name same as pstrSourceName of API pw_mp_AddDataSource
