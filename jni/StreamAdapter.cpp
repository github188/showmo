...
void StreamAdapter::setGlPpi(float ppi){
	memset(ppivalue,0,sizeof(char)*64);
	sprintf(ppivalue,"%d",(int)ppi);
	LOGE("setGlPpi:%s",ppivalue);
	//pstMontagePlay->pw_mp_SetInfo(lSourceID,"pixel_per_inche",ppivalue);
}
...
void StreamAdapter::InitPlayerGL(){
	if(pstMontagePlay != NULL){
		...
		bRes &= pstMontagePlay->pw_mpgl_SetInfo( "show_time_board", "y" );
		bRes &= pstMontagePlay->pw_mpgl_SetInfo( "effect", "lv0" );
		LOGE("PPI-->%s",ppivalue);
		pstMontagePlay->pw_mpgl_SetInfo( "pixel_per_inche",ppivalue);

		bRes &= pstMontagePlay->pw_mpgl_SetInfo( "time_scaleinch_x", "0.5" );
		bRes &= pstMontagePlay->pw_mpgl_SetInfo( "time_scaleinch_y", "0.08" );
		bRes &= pstMontagePlay->pw_mpgl_SetInfo( "time_offinch_x", "0.1" );
		bRes &= pstMontagePlay->pw_mpgl_SetInfo( "time_offinch_y", "0.1" );
